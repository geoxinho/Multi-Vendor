      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-16">
        ...
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-3 mb-4">
            <RatingStars rating={product.rating} size="md" showValue count={product.numReviews} />
          ...
          <div className="flex items-center gap-3 mb-6">
            <span className={`text-sm font-medium ${product.stock > 0 ? "text-green-600" : "text-red-500"}`}>
              {product.stock > 0 ? <><i className="fa-solid fa-check" /> {product.stock} in stock</> : "Out of stock"}
            </span>
          </div>
          ...
          <div className="mt-8 p-4 bg-gray-50 rounded-2xl border border-gray-100">
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center shrink-0">
                <span className="text-green-700 font-bold">{product.seller?.name?.[0]?.toUpperCase()}</span>
              </div>
              <div>
                ...
                {product.seller?.storeDescription && (
                  <p className="text-xs text-gray-500 mt-0.5 line-clamp-2">{product.seller.storeDescription}</p>
                )}
              </div>
            </div>
          </div>
